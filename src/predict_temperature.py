...
def predict_remaining_day_structured(room_name, day_name, current_hour, model, data):
    all_hours = [
        "09:00",
        "10:00",
        "11:00",
    ...
    for hour in all_hours[start_idx:]:
        ...
        temp_df = temp_df.rename(
            columns={
                ...
                "capacity_percentage": "occupation_%",
                "temperature_outdoor": "température_ext",
            }
        )

        temp_df = pd.get_dummies(temp_df, columns=["room"], prefix="salle")
        ...
        predicted_temp = round(model.predict(temp_df)[0], 1)

        predictions["room"]["days"][day_name]["temperature"][hour] = predicted_temp
        predictions["room"]["days"][day_name]["humidity"][hour] = float(
            row.get("humidity", 0)
        )
        ...
        row["temperature"] = predicted_temp
